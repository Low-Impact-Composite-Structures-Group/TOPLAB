...
def plot_g10_properties(temperature_range=(4, 300), num_points=1000):
    ...
    axes[1, 1].loglog(T, cp, 'g-', linewidth=2)
...
def get_all_coefficients():
    ...
    """
    return {
        'thermal_conductivity_normal': THERMAL_CONDUCTIVITY_NORMAL_COEFFS,
        'thermal_conductivity_warp': THERMAL_CONDUCTIVITY_WARP_COEFFS,
...
if __name__ == "__main__":
    ...
    print("G10 Material Properties Calculator")
    print("=" * 40)

    # Test at a few temperatures
    test_temps = [4, 10, 50, 77, 150, 300]  # Kelvin

    print(f"{'Temperature (K)':<15} {'k_normal (W/m·K)':<18} {'k_warp (W/m·K)':<17} {'Cp (J/kg·K)':<15}")
    print("-" * 70)
    ...
    for T in test_temps:
        ...
        cp = specific_heat(T)
        print(f"{T:<15.0f} {k_n:<18.6f} {k_w:<17.6f} {cp:<15.1f}")
