...
class MissionAdaptivePressureValve(InterTankCoupling):
    ...
    def calculate_pid_flow_rate(self, current_pressure_pa: float, target_pressure_pa: float, time: float) -> float:
        ...
        # Gain scheduling: reduced gains for gentler startup, full gains for steady-state
        controller_active_time = time - 100.0  # Time since controller activation
        if controller_active_time < 60.0:  # First 60 seconds after activation
            # Gentle startup gains (50% of nominal)
            gain_factor = 0.5
        else:
    ...
    def update_continuous_control(self, time: float, lh2_pressure: float, lh2_density: float):
        ...
        # Special startup stabilization mode - bypass PID control entirely for smooth settling
        if time < 300:  # First 5 minutes - pure stabilization mode
            startup_stabilization_mode = True
            deadband_bar = 1.0  # Very large deadband for maximum stability
        elif time < 600:  # 5-10 minutes - transition to normal control
            startup_stabilization_mode = False
            deadband_bar = 0.3  # Moderate deadband during transition
        else:
            startup_stabilization_mode = False
            deadband_bar = 0.1  # Normal deadband for precise control
