...
class MultiTankState:
    ...
    def state_vector(self) -> np.ndarray:
        ...
        for tank_state in self.tank_states:
            vector.extend([
                ...
                tank_state.h2_temperature,
                tank_state.structure_temperature,
                tank_state.insulation_temperature,
    ...
    def from_state_vector(cls,
        ...
        if len(state_vector) != 5 * n_tanks:
            raise ValueError(f"State vector length {len(state_vector)} != 5 * {n_tanks} tanks")
        ...
        for i in range(n_tanks):
            ...
            tank_state = IsochoricTankState(
                ...
                h2_temperature=state_vector[idx + 1],
                structure_temperature=state_vector[idx + 2],
                insulation_temperature=state_vector[idx + 3],
                shell_temperature=state_vector[idx + 4],
            )
    ...
    def update_from_state_vector(self, state_vector: np.ndarray):
        ...
        for i, tank_state in enumerate(self.tank_states):
            ...
            tank_state.h2_temperature = state_vector[idx + 1]
            tank_state.structure_temperature = state_vector[idx + 2]
            tank_state.insulation_temperature = state_vector[idx + 3]
            tank_state.shell_temperature = state_vector[idx + 4]
...
class MultiTankResults:
    ...
    def get_combined_data(self) -> Dict[str, np.ndarray]:
        ...
        for tank_idx in range(self.n_tanks):
            ...
            for key in ['masses', 'h2_temperatures', 'structure_temperatures', 'insulation_temperatures', 'shell_temperatures',
    ...
    def _extract_tank_arrays(self, tank_index: int) -> Dict[str, np.ndarray]:
        ...
        for multi_state in self.multi_tank_states:
            ...
            h2_temperatures.append(tank_state.h2_temperature)
            structure_temperatures.append(tank_state.structure_temperature)
            insulation_temperatures.append(tank_state.insulation_temperature)
        ...
        return {
            ...
            'h2_temperatures': np.array(h2_temperatures),
            'structure_temperatures': np.array(structure_temperatures),
            'insulation_temperatures': np.array(insulation_temperatures),
