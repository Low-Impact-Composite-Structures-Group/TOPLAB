...
V1  Exact directional cylindrical + spherical radial conduction
...
V8  Transient insulation energy balance
...
from toplab.materials.rohacell_properties import integrated_thermal_conductivity
...
def _make_model(
    ...
    foam_mass = _layer_mass(ROHACELL_DENSITY, r_s, r_sh)
...
def _make_state(
    ...
    return IsochoricTankState(
        ...
        fuel_mass=mass,
        h2_temperature=T_h2,
        structure_temperature=T_structure,
        insulation_temperature=T_insulation,
...
def _q_radial_integral_ref(
    ...
    Q_cyl  = 2π L / ln(r_cyl_outer / r_cyl_inner) · ∫ k(T)dT
    Q_caps = 4π r_sph_inner r_sph_outer / (r_sph_outer − r_sph_inner) · ∫ k(T)dT

    Positive heat flow is from the outer temperature to the inner temperature.
    """
    integrated_k = integrated_thermal_conductivity(T_inner, T_outer)
    Q_cyl = 2.0 * math.pi * L * integrated_k / math.log(r_cyl_outer / r_cyl_inner)
    Q_cap = 4.0 * math.pi * r_sph_inner * r_sph_outer * integrated_k / (r_sph_outer - r_sph_inner)
    return Q_cyl + Q_cap
...
class TestV1ExactRadialConduction:
    """
    Independently evaluate each half-layer Fourier integral and compare against
    the directional production heat-flow methods.

    A pass at machine precision (< 1 × 10⁻¹²) confirms that the production
    formula matches the governing equations.
    """

    def test_directional_heat_fluxes_match_independent_integrals(self):
        model = _make_model()
        state = _make_state()
        Q_shell_ref = _q_radial_integral_ref(
            model.r_m_cyl, model.r_shell, model.r_m_sph, model.r_shell, model.L,
            state.insulation_temperature, state.shell_temperature,
        )
        Q_shell_prod = model.compute_shell_to_insulation_heat_flux(
            state.shell_temperature, state.insulation_temperature
        )
        Q_structure_ref = _q_radial_integral_ref(
            model.r_structure, model.r_m_cyl, model.r_structure, model.r_m_sph, model.L,
            state.structure_temperature, state.insulation_temperature,
        )
        Q_structure_prod = model.compute_insulation_to_structure_heat_flux(
            state.insulation_temperature, state.structure_temperature
        )
        assert abs(Q_shell_prod - Q_shell_ref) / abs(Q_shell_ref) < 1e-12
        assert abs(Q_structure_prod - Q_structure_ref) / abs(Q_structure_ref) < 1e-12

    def test_cylindrical_contribution_positive_when_shell_hotter(self):
        model = _make_model()
        T_m = 0.5 * (_T_STRUCTURE_OP + _T_SHELL_OP)
        k = rohacell_k(T_m)
        Q_cyl = 2.0 * math.pi * model.L * k / math.log(model.r_shell / model.r_m_cyl) * (_T_SHELL_OP - 174.0)
        assert Q_cyl > 0.0

    def test_endcap_contribution_positive_when_shell_hotter(self):
        model = _make_model()
        r_s, r_sh = model.r_m_sph, model.r_shell
        T_m = 0.5 * (174.0 + _T_SHELL_OP)
        k = rohacell_k(T_m)
        Q_cap = 4.0 * math.pi * k * r_s * r_sh / (r_sh - r_s) * (_T_SHELL_OP - 174.0)
        assert Q_cap > 0.0
...
class TestV2ThinInsulationAsymptotic:
    ...
    def _q_planar(t_ins: float, T_s: float, T_sh: float) -> float:
        """Planar-wall integral reference at the structure surface area."""
        A = 2.0 * math.pi * _R_STRUCTURE * _L_CYL + 4.0 * math.pi * _R_STRUCTURE**2
        integrated_k = integrated_thermal_conductivity(T_s, T_sh)
        return A / t_ins * integrated_k

    @staticmethod
    def _q_radial(t_ins: float, T_s: float, T_sh: float) -> float:
        r_sh = _R_STRUCTURE + t_ins
        return _q_radial_integral_ref(
            _R_STRUCTURE, r_sh, _R_STRUCTURE, r_sh, _L_CYL, T_s, T_sh
        )
...
class TestV3MeanTemperatureConductivity:
    ...
    def test_conductivity_approximation_error(self, T_lo, T_hi):
        ...
        integrated_k = integrated_thermal_conductivity(T_lo, T_hi)
...
class TestV4ShellEnergyBalance:
    ...
    def test_shell_derivative_matches_independent_energy_balance(self):
        model = _make_model()
        state = _make_state()
        T_s = state.structure_temperature
        ...
        Q_ins_ref = _q_radial_integral_ref(
            model.r_m_cyl, model.r_shell, model.r_m_sph, model.r_shell, model.L,
            state.insulation_temperature, T_sh,
        )
...
class TestV5StructureEnergyBalance:
    ...
    def test_structure_derivative_matches_independent_energy_balance(self):
        ...
        Q_ins_ref = _q_radial_integral_ref(
            model.r_structure, model.r_m_cyl, model.r_structure, model.r_m_sph, model.L,
            T_s, state.insulation_temperature,
        )
        ...
        alpha_s = model.get_alpha_s(state.h2_temperature, T_s, state.pressure)
        Q_str_ref = alpha_s * model.A_in * (T_s - state.h2_temperature)
    ...
    def test_q_insulation_enters_structure_positively(self):
        ...
        Q_ins = _q_radial_integral_ref(
            _R_STRUCTURE, math.sqrt(_R_STRUCTURE * _R_SHELL),
            _R_STRUCTURE, 2.0 * _R_STRUCTURE * _R_SHELL / (_R_STRUCTURE + _R_SHELL),
            _L_CYL, _T_STRUCTURE_OP, 174.0,
        )
        assert Q_ins > 0.0
    ...
    def test_q_structure_leaves_structure_positively(self):
        ...
        model = _make_model()
        state = _make_state()
        Q_str = model.compute_structure_to_h2_heat_flux(0.0, state)
        assert Q_str > 0.0
...
class TestV7SteadyStateConservation:
    ...
    def test_energy_balance_residuals_at_steady_state(self):
        ...
        def _residual(T_vec):
            T_sh, T_ins, T_s = T_vec
            Q_amb = model.compute_ambient_to_shell_heat_flux(T_sh)
            Q_shell_to_ins = model.compute_shell_to_insulation_heat_flux(T_sh, T_ins)
            Q_ins_to_structure = model.compute_insulation_to_structure_heat_flux(T_ins, T_s)
            Q_str = alpha_s * model.A_in * (T_s - _T_FLUID_OP)
            return [Q_amb - Q_shell_to_ins, Q_shell_to_ins - Q_ins_to_structure,
                    Q_ins_to_structure - Q_str]
        ...
        x0 = [model.T_amb - 15.0, 0.5 * (model.T_amb + _T_FLUID_OP), _T_FLUID_OP + 8.0]
        T_ss, _info, ier, msg = fsolve(_residual, x0, full_output=True)
        assert ier == 1, f"V7: Steady-state solver failed to converge: {msg}"

        T_sh_ss, T_ins_ss, T_s_ss = T_ss
        Q_amb_ss = model.compute_ambient_to_shell_heat_flux(T_sh_ss)
        Q_shell_to_ins_ss = model.compute_shell_to_insulation_heat_flux(T_sh_ss, T_ins_ss)
        Q_ins_ss = model.compute_insulation_to_structure_heat_flux(T_ins_ss, T_s_ss)
        Q_str_ss = alpha_s * model.A_in * (T_s_ss - _T_FLUID_OP)

        scale = abs(Q_amb_ss)
        R_shell = abs(Q_amb_ss - Q_shell_to_ins_ss) / scale
        R_insulation = abs(Q_shell_to_ins_ss - Q_ins_ss) / scale
        ...
        assert R_insulation < 1e-8, f"V7: Insulation residual {R_insulation:.2e} at steady state."


# ===========================================================================
# V8 — Transient foam capacity
# ===========================================================================
...
class TestV8TransientFoam:
    """
    Verify that the foam state has a finite positive thermal capacity and that its
    directional heat flows are consistent with a transient internal-energy balance.
    """
    ...
    def _compute_system_timescales(self, model):
        ...
        T_ins = 174.0
        G_foam = model.compute_shell_to_insulation_heat_flux(T_sh, T_ins) / (T_sh - T_ins)
        h_A = model.alpha_amb * model.A_shell
        t_foam = model.r_shell - model.r_structure
        return C_shell, C_structure, G_foam, h_A, t_foam

    def test_foam_thermal_capacity_is_positive(self):
        """
        ...
        assert result["tau_foam"] > 0.0

    def test_foam_temperature_derivative_matches_internal_energy_balance(self):
        model = _make_model()
        C_shell, C_structure, G_foam, h_A, t_foam = self._compute_system_timescales(model)

        state = _make_state()
        heat_capacity = model.m_foam * rohacell_cp(state.insulation_temperature)
        expected = (
            model.compute_shell_to_insulation_heat_flux(state.shell_temperature, state.insulation_temperature)
            - model.compute_insulation_to_structure_heat_flux(state.insulation_temperature, state.structure_temperature)
        ) / heat_capacity
        assert model.compute_insulation_temperature_derivative(0.0, state) == pytest.approx(expected)
