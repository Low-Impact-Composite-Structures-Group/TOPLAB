...
def perform_analysis():
    ...
    def objective_function(params):
        ...
        all_thicknesses.append(insulation_thickness)
    ...
    if not RESPONSE_SURFACE:
        ...
        result = minimize(objective_function, initial_guess, method='Nelder-Mead', bounds=opt_bounds)
        optimal_radius, optimal_insulation_thickness = result.x
        print(f'Optimal radius: {optimal_radius} m')
        print(f'Optimal thickness: {optimal_insulation_thickness} m')
        print(f'Minimized gravimetric efficiency: {-result.fun}')

        optimal_insulation_thickness = ConstantFoamInsulation.rohacell(optimal_insulation_thickness)
        ...
        psi_values = [all_radii / all_b for all_radii, all_b in zip(all_radii, all_thicknesses)]
        ...
        ax.plot_surface(radius_grid, thickness_grid, gravimetric_efficiency_grid, cmap='viridis')

        ax.set_xlabel('Radius')
        ax.set_ylabel('Insulation Thickness')
